...
const Signup = () => {
  ...
  const {user, setUser, setCart, setUserLoading, CheckUser} = useContext(AppContext);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${backendUrl}/api/auth/user/signup`, inpValue);
  ...
  return (
    <div className="w-full h-full xl:mt-16  xl:flex xl:flex-col items-center justify-center dark:bg-black dark:text-white  px-6 overflow-y-auto  noScroll">
      ...
      <form
        ...
        className="mt-8 space-y-6  h-full lg:w-lg  w-full   noScroll"
        noValidate>
        {errorMessage && (
          <div
